  ...
  function _doSafeTransferAcceptanceCheck(
    ...
    if (to.code.length > 0) {
      if (IERC165(to).supportsInterface(type(IERC1155).interfaceId)) {
        ...
        try
          ERC721Receiver(to).onERC721Received(operator, from, id, data)
        returns (bytes4 response) {
      ...
      if (mint) {
        ...
        if (!whitelist[to]) {
          if (easyLaunch == 1 && wlf && from == owner()) {
            ...
            uint256 tokens_to_mint = ((toBalance + value) / tokensPerNFT) -
              (toBalance / tokensPerNFT);
  ...
  function _afterTokenTransfer(
    ...
    if (!whitelist[to]) {
      ...
      if (transferDelay) {
        ...
        require(
          address(to).code.length == 0 && address(tx.origin).code.length == 0,
